...
--100 means total---
--select ename,count(*) as Total_Employee  from emp where datediff(yy,hiredate,getdate()) between 30 and 40 group by rollup(ename) 
...
--select count(*) as Total_Number from emp where datediff(yy,hiredate,getdate()) between 30 and 40 
...
--select * from dept d left join emp e on d.deptno=e.deptno order by d.dname, e.ename desc
